...
const BUILTIN: Omit<PromptTemplate, "createdAt" | "updatedAt" | "builtin">[] = [
  ...
  },
  {
    ...
    taskId: "coding",
    prompt:
      "请把以下代码重构为单一职责，保留外部行为不变，并说明每一处改动的原因：\n\n```ts\n// 粘贴代码\n```",
  },
  {
    id: "coding-bug",
    title: "排查 Bug",
  ...
  },
  {
    ...
    taskId: "code_review",
    prompt:
      "请从安全角度审查以下代码，重点关注注入、权限、敏感信息泄露、输入校验和重放风险：\n\n```ts\n// 粘贴代码\n```",
  },
  {
    id: "doc-analysis-default",
    title: "需求大纲",
    taskId: "document_analysis",
    prompt:
      "请基于上传的需求文档，生成一份系统设计方案大纲，包括架构图（Mermaid）、核心模块说明、技术栈建议和接口定义。",
  },
  {
  ...
  },
  {
    ...
    taskId: "document_analysis",
    prompt:
      "请基于文档内容识别潜在的实现风险、依赖风险和时间风险，并给出建议的行动项和负责人（如可推断）。",
  },
  {
  ...
  },
  {
    ...
    taskId: "prompt_optimize",
    prompt:
      "请为以下场景设计一个专家角色（背景、立场、擅长领域、回答风格），并把现有提示词改写为带角色扮演的版本。",
  },
];
...
export function saveUserTemplate(input: SaveTemplateInput): SaveResult {
  ...
  if (input.prompt.length === 0 || input.prompt.length > 32000)
...
export function importUserTemplates(json: string, mode: "skip" | "overwrite"): ImportResult {
  ...
  const byKey = new Map<string, PromptTemplate>(
    existing.map((t) => [`${t.taskId}::${t.title}`, t]),
  );
