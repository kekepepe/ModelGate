...
import { Code2, FileText, MessageSquare, Sparkles, Video, WandSparkles, Play } from "lucide-react";
  ...
  const capabilityCards = [
    {
      icon: MessageSquare,
      title: "Chat",
      desc: "Multi-turn conversation with AI models",
      href: "/workspace?taskType=chat",
    },
    {
      icon: Code2,
      title: "Coding",
      desc: "Code generation, review and debugging",
      href: "/workspace?taskType=coding",
    },
    {
      icon: FileText,
      title: "Document Analysis",
      desc: "Extract insights from documents and files",
      href: "/workspace?taskType=document_analysis",
    },
    {
      icon: Sparkles,
      title: "Prompt Optimize",
      desc: "Improve and refine your prompts",
      href: "/workspace?taskType=prompt_optimize",
    },
    {
      icon: WandSparkles,
      title: "Generation",
      desc: "Text-to-image and text-to-video generation",
      href: "/workspace?taskType=text_to_image",
    },
    {
      icon: Video,
      title: "Video Understanding",
      desc: "Analyze and understand video content",
      href: "/workspace?taskType=video_understanding",
    },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard
          ...
          sub={`${configuredProviders} configured`}
        />
        ...
        <StatCard
          ...
          sub={
            runs.length > 0 ? `${((failedRuns / runs.length) * 100).toFixed(1)}% rate` : "0% rate"
          }
        />
      </div>
            ...
            {providers.length > 0 ? (
              providers.map((provider) => (
                <div key={provider.id} className="flex items-center justify-between px-4 py-3">
                  <div className="flex items-center gap-3">
                    <div
                      className={`h-2 w-2 rounded-full ${provider.configured ? "bg-emerald-500" : "bg-amber-500"}`}
                    />
                    <div>
          ...
          <div className="divide-y">
            {recentRuns.length > 0 ? (
              ...
              <div className="p-4 text-sm text-muted-foreground">
                No runs yet. Try the Playground!
              </div>
            )}
          </div>
        </div>
