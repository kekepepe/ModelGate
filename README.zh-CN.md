...
> 本地优先的多模型 AI 工作台，统一接入 token plan 类厂商。

**[English](README.md)** · **[简体中文](README.zh-CN.md)**

ModelGate 是一个自托管、单用户的 AI 工作台，把多家"token plan"类
模型厂商（Xiaomi MiMo、MiniMax、火山引擎 Coding Plan、火山引擎
Seedance 等）统一在一套具备能力感知的 UI、一份共享历史、一组统一
的安全保证后面。它是你和模型订阅之间缺失的那一层。

它**不是** LLM 代理、IDE 插件、纯聊天 Playground，也不是 SaaS。它
是一台"个人指挥中心"，把"我手上有五份 token plan"变成"我可以让
其中任何一个回答任何问题，并且相信它的返回"。
...
| **同步 Chat 运行时 + 异步生成运行时** | 闲聊走 ChatRuntime，生成视频/图片走 GenerationRuntime，两边共享同一套存储、日志、取消链路。 |
...
- **先做聊天/代码模型。** 支持视觉的模型已经能直接吃图，模型注册表按能力过滤。
- **已覆盖厂商：** Xiaomi MiMo、MiniMax、火山引擎 Coding Plan。火山引擎 Seedance 视频生成由 `MODELGATE_ENABLE_SEEDANCE=true` 开关控制，代码完整，默认关闭。
- **第一版**不**包含：** Workflow、多模型对比、OpenAI 兼容入站端点、MCP Server、多租户鉴权——这些留给 Phase 10。数据库 schema 和 feature flag 都已经为它们留好位置。

完整决策见 [`docs/00-项目概览/第一版范围与开发决策.md`](docs/00-项目概览/第一版范围与开发决策.md)。
...
- **前端：** Next.js 15 · TypeScript · Tailwind CSS · shadcn/ui · Zustand · TanStack Query · React Hook Form + Zod
...
![ModelGate Playground 截图 — 任务类型、模型、参数 schema、输出面板](github-picture/github2.png)

Playground 是真正干活的地方。挑一个任务类型（聊天、写代码、代码
审查、文档分析、Prompt 优化……），从能回答这个任务的模型里挑一个，
再在由 `configs/param-schemas.json` 实时渲染出的参数面板里调参。
