...
- 同时使用 MiMo、MiniMax、火山 Coding Plan 等 token plan 的个人开发者。
- 需要在文本、代码、图片理解、生图、生视频、多 Agent 流程之间频繁切换的人。
- 希望保留官方模型名，但不想手动记忆每个模型能力、参数和 API 调用方式的人。
- 希望 clone GitHub 项目后，在本地配置 API Key 并快速使用多个 token plan 的人。
...
- 第一版 Provider 接入顺序：MiMo、MiniMax、火山 Coding Plan；火山 Seedance 暂不接入，仅预留。
- 第一版模型清单：MiMo-V2.5-Pro、MiMo-V2.5、MiniMax-M2.7-highspeed、MiniMax-M2.7、Kimi-K2.6、GLM-5.1、DeepSeek-V4-Pro、DeepSeek-V4-Flash、Doubao-Seed-2.0-Code、Doubao-Seed-2.0-pro。
- Provider 管理：MiMo、MiniMax、火山 Coding Plan；Moonshot / Kimi、Zhipu / GLM 直连接口后续按需补充。
...
- 按顺序接入 MiMo、MiniMax、火山 Coding Plan 的 Chat 类 Provider。
- 支持普通聊天、代码、图片理解、文档分析。
- 流式输出不作为第一版硬性要求；如实现难度较高，先支持非流式。
- 保存 run、request_log、usage_log。

### Phase 4：Generation Runtime 预留

- 保留 generation_task、Worker、状态机和 Adapter 结构。
- 火山 Seedance 第一版暂不接入。
- MiniMax 图像或视频生成能力后续扩展。
- 第一版不把文生视频、图生视频作为验收项。
