...
- 支持 Chat 流式输出：`/api/chat/runs/stream` 返回 SSE，OpenAI-compatible Provider 解析 `chat/completions` streaming delta。
- 支持文件预处理后的上下文注入。
- 支持任务级 system prompt：`chat`、`coding`、`code_review`、`document_analysis`、`prompt_optimize` 会分别注入身份定位、能力边界、工作方式和输出规范。
- 文件解析内容只能进入 user message 的 `BEGIN_USER_FILE_CONTEXT` 边界块，不能进入 system prompt。
- 保存 run 记录。
