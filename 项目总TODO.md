...
- [X] 选择聊天任务并运行。
  状态：API 级 E2E 已覆盖；真实浏览器 E2E 后续可接 Playwright。
- [ ] 上传图片并推荐图片理解模型。
...
- [X] Provider 返回错误时前端正确展示。
  状态：前端 API error 已携带 `type/status/requestId` 并在工作台展示 requestId；真实浏览器 E2E 后续可接 Playwright。
- [X] 取消任务时状态正确。
...
- [ ] 本地 Docker Compose 一键启动。
  状态：`docker compose config --quiet` 已通过；完整 compose 一键启动未在本轮重建，以免打断当前本地服务。
- [X] README 可指导新环境启动项目。
...
- [X] Phase 9：测试与验收。
  状态：Model Registry / Capability Router、Provider Adapter、文件上传、模型推荐、Chat Runtime、Generation Task、历史记录、日志、安全边界和前端错误展示验收链路已覆盖；真实视觉模型、Seedance 真实任务和完整 Docker Compose 重建留作后续外部条件验收。
...
6. [ ] 后续补 Phase 6 增强：流式输出、运行中取消、图片多模态输入。
7. [ ] 后续按外部条件补视觉模型验收、Seedance 真实生成、完整 Docker Compose 一键重建。
