...
- [X] 前端视觉参考改造计划
  产出：[ModelGate_前端视觉参考改造计划.md](./ModelGate_前端视觉参考改造计划.md)
...
## 12. Phase 11：前端视觉参考改造

目标：参考 `前端设计参考图/` 中 5 张页面截图的布局、留白、边框、卡片层级、左侧导航、中央输入工作台、参数弹窗、用量统计和日志表格，把 ModelGate 改造成多模型 AI API 工作台。只参考结构和交互语言，不复制 Firecrawl 品牌、Logo、橙色主色、文案或业务内容。

设计计划：[ModelGate_前端视觉参考改造计划.md](./ModelGate_前端视觉参考改造计划.md)

### 12.1 统一应用外壳

- [ ] 新建或重构 `AppShell`，统一 Sidebar、Topbar、页面主内容区。
- [ ] Sidebar 改成 ModelGate 导航：Overview、Playground、Models、Usage、Activity Logs、API Keys、Settings。
- [ ] 顶部工具条保留 Local Workspace、全局搜索、健康状态、设置入口。
- [ ] 统一浅色控制台视觉 tokens：背景、边框、文字、主色、状态色。
- [ ] 避免 Firecrawl 品牌痕迹和橙色主色主导。
...
- [ ] 展示 requests、tokens、estimated cost、average latency、failure rate、concurrent / queued tasks。
- [ ] 支持 Weekly / Monthly、Provider、Model、Task type 过滤。
- [ ] 图表区域按参考图做大留白和清晰边框。

### 12.4 Activity Logs 日志页

- [ ] 将历史和请求日志整理成 Activity Logs 页面。
- [ ] 表格字段覆盖 Time、Task、Provider、Model、Status、Latency、Tokens、Cost、Request ID、Actions。
- [ ] 支持 requestId / provider / model / task / error type 搜索。
- [ ] 支持 Provider、Model、Status、Date range 过滤。
- [ ] 行详情 Sheet 展示脱敏 request、response、error mapping、关联 run / generation task。

### 12.5 API Key 与 Provider 状态

- [ ] API Key 页面或设置区展示 Provider 配置状态、来源和最后更新时间。
- [ ] API Key 只允许写入，不回显明文。
- [ ] Provider 状态卡展示 configured / healthy / degraded / missing。
- [ ] 写入密钥后触发 Provider status refresh。

### 12.6 视觉验收

- [ ] Playwright 桌面端截图检查，建议视口接近参考图 `1460px` 宽。
- [ ] Playwright 移动端截图检查，确认 Sidebar、参数弹窗、表格不会重叠。
- [ ] 检查所有页面没有 Firecrawl Logo、业务文案和品牌色照搬。
- [ ] 复测核心链路：选择任务、推荐模型、调整参数、运行、取消、查看历史和日志。
...
- [X] Phase 11：前端视觉参考改造计划。  
  状态：已根据 `前端设计参考图/` 5 张截图整理转译原则、页面规划、组件拆分、实施顺序和验收标准；实现项尚未开始。
...
8. [ ] 进入 Phase 11 前端视觉参考改造：先做统一应用外壳和 Playground 中央工作台。
