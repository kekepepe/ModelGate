...
- [ ] 错误码与日志规范建议文件名：`错误码与日志规范.md`内容范围：标准错误类型、错误响应格式、requestId、日志脱敏、Provider 错误映射。
- [ ] 前端状态管理与页面交互规范建议文件名：`前端状态管理与页面交互规范.md`内容范围：Zustand store 拆分、TanStack Query 管理范围、任务切换、模型切换、文件删除、任务取消的状态重置规则。
- [ ] 环境变量与部署文档建议文件名：`环境变量与部署文档.md`内容范围：`.env.example`、Docker Compose、PostgreSQL、Redis、目录挂载、本地/测试/生产环境差异。
- [ ] 测试策略文档建议文件名：`测试策略文档.md`内容范围：单元测试、集成测试、E2E 测试；Adapter mock、Capability Router fixture、文件上传、任务流测试。
- [ ] 开发规范文档
  建议文件名：`开发规范文档.md`
